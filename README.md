...
| OpenCLI | 见 `scripts/docker/opencli-artifact.lock.env`（当前 fork `1.8.7-fengwk.7`） |
| Browser Bridge extension | 见同一 lock（当前 fork `1.0.28`） |
| MySQL | `5.7.44`（已 EOL） |
...
3. **默认 lock 只指向已发布资产**：当前钉住已验证的 `fork-v1.8.7-fengwk.7`；不要提交未发布的本地产物 URL。
