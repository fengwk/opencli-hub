-- MySQL 5.7-compatible fresh schema, idempotent for Spring SQL init (schema only; no data SQL).
--
...
--
-- state_changed_at must NOT use ON UPDATE: state transitions are written by the application
-- with explicit timestamps, and DB-side auto-update would silently rewrite the sort key.
-- Existing MySQL DBs are not altered by this file; keep using scripts/migrate-mysql-*.sql.
