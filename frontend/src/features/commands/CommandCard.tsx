...
interface CommandCardProps {
  command: HubCommand
  busy: boolean
  onBlacklist: (command: HubCommand, reason: string) => Promise<void>
  onUnblacklist: (command: HubCommand) => Promise<void>
  onSaveOutputRule: (command: HubCommand, rule: OutputRuleUpdate) => Promise<void>
  onDeleteOutputRule: (command: HubCommand) => Promise<void>
}
...
export function CommandCard({
  ...
  async function submitOutputRule(event: FormEvent<HTMLFormElement>) {
    ...
    setValidationError(null)
    await onSaveOutputRule(command, {
      argumentName,
      targetType,
      fileName: targetType === 'FILE' ? normalizedFileName : null,
    })
    setEditingOutputRule(false)
  }
  ...
  return (
    <article className="command-card">
      ...
      <dl className="metadata-grid">
        ...
        <div><dt>别名</dt><dd>{command.aliases?.join(', ') || '—'}</dd></div>
        <div><dt>浏览器命令</dt><dd>{command.browser ? '是' : '否'}</dd></div>
      ...
      <section aria-label={`${command.commandKey} 黑名单`} className="command-section policy-section">
        ...
        {command.blacklisted ? (
          ...
          <div className="inline-form">
            ...
            <button type="button" className="btn btn-danger" disabled={busy} onClick={() => void submitBlacklist()}>加入黑名单</button>
          </div>
        )}
      </section>
      ...
      <section aria-label={`${command.commandKey} 输出规则`} className="command-section policy-section">
        <div className="section-heading-row">
          <h3>输出规则</h3>
          {command.outputRule ? (
            <button type="button" className="btn" disabled={busy} onClick={() => void onDeleteOutputRule(command)}>删除输出规则</button>
          ) : null}
        </div>
        {command.outputRule ? (
          <p>参数 <strong>{command.outputRule.argumentName}</strong> → {command.outputRule.targetType}{command.outputRule.fileName ? `（${command.outputRule.fileName}）` : ''}</p>
        ) : <p className="muted">尚未配置输出资源规则。</p>}
