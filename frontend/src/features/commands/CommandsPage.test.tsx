...
/** Existing rule whose `op` argument is Hub-managed and therefore absent from args. */
...
describe('CommandsPage', () => {
  ...
  it('shows an existing output rule with a hidden managed argument and saves/deletes it', async () => {
    // The rule's argument `op` is platform-managed, so it is absent from public args;
    // the catalog must still surface the rule, count it in metrics, and let the editor
    // keep and submit that argument name without a false incompatibility warning.
    const user = userEvent.setup()
    vi.mocked(apiClient.get).mockResolvedValue([managedRuleCommand])

    renderPage()
    await screen.findByRole('heading', { name: 'demo/search' })

    const metrics = screen.getByLabelText('命令目录概览')
    expect(within(metrics).getByText('输出规则').parentElement).toHaveTextContent('输出规则1')
    ...
    await waitFor(() => expect(apiClient.put).toHaveBeenCalledWith(
      '/opencli/commands/demo/search/output-rule',
      { argumentName: 'op', targetType: 'FILE', fileName: 'result.json' },
    ))
