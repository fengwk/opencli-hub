...
const instance: HubInstance = {
  id: 42, code: 'beta', displayName: 'Beta browser', contextId: 'ctx-42', state: 'RUNNING',
...
const vncStatus: HubInstanceVncStatus = {
  instanceId: 42, instanceAvailable: true, running: true, runtimeAvailable: true, vncAvailable: true,
}
...
function renderPage() {
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <MemoryRouter initialEntries={['/instances/42']}>
...
describe('InstanceDetailPage', () => {
  it('renders one instance, its VNC availability, and the instance-filtered logs link', async () => {
    // Route-specific responses prove the detail page queries both REST contracts rather than deriving VNC availability locally.
    vi.mocked(apiClient.get).mockImplementation((url: string) => Promise.resolve(
      url === '/instances/42/vnc/status' ? vncStatus : instance,
    ) as never)
    ...
    expect(screen.getByRole('link', { name: '查看日志' })).toHaveAttribute('href', '/logs?instanceId=42')
    expect(apiClient.get).toHaveBeenCalledWith('/instances/42/vnc/status')
  })
  ...
  it('saves edits through the exact update route and refreshes the instance', async () => {
    ...
    vi.mocked(apiClient.get).mockImplementation((url: string) => Promise.resolve(
      url === '/instances/42/vnc/status' ? vncStatus : url === '/opencli/commands' ? [{
    ...
    await waitFor(() => expect(apiClient.put).toHaveBeenCalledWith('/instances/42', {
